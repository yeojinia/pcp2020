...
public:
	...
	my_vec::my_vec2 _gravitationCenter;     // Gravitation center.
...
public:
	 ...
	 */
	void get_bounding_box(my_vec::my_vec2 &bottomLeft_,
	 ...
	 */
	//void draw();
	void draw(int option); // option 1: hierarchical only, option 2: hierarchical + bezier curve
	std::vector<anchor_pts> get_three_points();
	std::vector<my_vec::my_vec2> get_last_points();
	...
	std::pair<double, double> get_gravitational_center_of_cluster(int edge_num);
